...
- A functional component does not manage its own state and is not able to keep track of changing data.
- A stateful component has state and is able to keep track of changing data.

2. When does a componentWillMount function be called? What about a componentDidUpdate?

- The `componentWillMount` function is called before the render method is executed.The `componentWillUpdate` function is called as soon as the `shouldComponentUpdate` returns true. Any state changes through `this.setState` are not allowed because it is used for an upcoming update
